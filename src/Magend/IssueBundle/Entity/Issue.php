...
class Issue
{
    ...
    public function prePersist()
    {
       ...
       if ($this->landscapeCoverFile) {
        ...
        if ($this->previewFile) {
            $imgName = uniqid('preview_') . '.' . $this->previewFile->guessExtension();
            $this->previewFile->move(__DIR__.'/../../../../web/uploads/', $imgName);
            
            if ($this->getPreview()) {
                @unlink(__DIR__.'/../../../../web/uploads/' . $this->getPreview());
            }
            
            $this->setPreview($imgName);
        }
        
        if ($this->enPreviewFile) {
            $imgName = uniqid('enpreview_') . '.' . $this->enPreviewFile->guessExtension();
            $this->enPreviewFile->move(__DIR__.'/../../../../web/uploads/', $imgName);
        
            if ($this->getEnPreview()) {
                @unlink(__DIR__.'/../../../../web/uploads/' . $this->getEnPreview());
            }
        
            $this->setEnPreview($imgName);
        }
